...
class MetricsLogger:
    ...
    def log_packet_loss(self, loss_rate: float):
        ...
        t = time.time() - self.start_time
        self.data["packet_loss"].append({"t": round(t, 4), "loss": round(loss_rate, 6)})
    ...
    def log_bandwidth(self, bytes_sent: int, bytes_recv: int):
        ...
        self.data["bandwidth"].append(
            {"t": round(t, 4), "sent_bytes": bytes_sent, "recv_bytes": bytes_recv}
        )
    ...
    def get_summary(self) -> dict:
        ...
        def _percentile(data_sorted: list[float], p: float) -> float:
            if not data_sorted or len(data_sorted) == 0:
                return 0.0
        ...
        if rtts:
            ...
            summary["rtt_p50"] = _percentile(rtts_sorted, 0.50)
            summary["rtt_p95"] = _percentile(rtts_sorted, 0.95)
            summary["rtt_p99"] = _percentile(rtts_sorted, 0.99)
