...
def merge_metrics(*datasets: dict) -> dict:
    """Merge multiple metrics dicts by concatenating each list field."""
    merged = {
        "rtt": [],
        "jitter": [],
        "packet_loss": [],
    ...
    # Sort time-based series by timestamp
    for key in ("rtt", "jitter", "packet_loss", "bandwidth", "prediction_error"):
        if merged[key] and "t" in merged[key][0]:
            merged[key].sort(key=lambda x: x["t"])
    # Sort tick_times by tick number
    if merged["tick_times"]:
        merged["tick_times"].sort(key=lambda x: x["tick"])
    return merged


def auto_discover_metrics(filepath: str) -> dict:
    """Load the given file and auto-discover companion metrics files.

    If filepath is a server metrics file, also look for client_*_metrics.json
    in the same directory and merge them.
    """
    data = load_metrics(filepath)
    log_dir = os.path.dirname(os.path.abspath(filepath))
    datasets = [data]

    # Look for companion metrics files
    import glob

    for pattern in ["client_*_metrics.json", "server_metrics.json"]:
        for path in glob.glob(os.path.join(log_dir, pattern)):
            abs_path = os.path.abspath(path)
            if abs_path == os.path.abspath(filepath):
                continue  # skip the file we already loaded
            try:
                companion = load_metrics(path)
                datasets.append(companion)
                print(f"[ANALYSIS] Also loaded: {os.path.basename(path)}")
            except (json.JSONDecodeError, OSError):
                pass

    if len(datasets) > 1:
        return merge_metrics(*datasets)
    return data
...
def plot_latency_analysis(data: dict, output_dir: str = "analysis"):
    ...
    fig.suptitle(
        "Real-Time Multiplayer Network Analysis", fontsize=14, fontweight="bold"
    )

    def _no_data_label(ax, title):
        """Place a centered 'No data' label on an empty subplot."""
        ax.text(
            0.5,
            0.5,
            "No data available",
    ...
    if rtts:
        ...
        _no_data_label(ax, "Round-Trip Time (RTT)")
    ...
    if jitters:
        ...
        ax.plot(
            jtimes, jvalues, linewidth=1.5, color="red", label="Smoothed (RFC 3550)"
        )
        ax.legend(fontsize=9)
        ax.set_title("Jitter")
    else:
        _no_data_label(ax, "Jitter")
    ax.set_xlabel("Time (s)")
    ...
    if rtts:
        ...
        _no_data_label(ax, "RTT Distribution")
    ...
    if losses:
        ...
        _no_data_label(ax, "Packet Loss Rate")
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Loss (%)")
    ax.grid(True, alpha=0.3)
...
def plot_tick_times(data: dict, output_dir: str = "analysis"):
    ...
    ax.axhline(y=mean_d, color="blue", linestyle="--", label=f"Mean: {mean_d:.3f} ms")
...
def analyze_all(filepath: str, output_dir: str = "analysis"):
    ...
    data = auto_discover_metrics(filepath)
    ...
    if errors:
        print(
            f"  Pred Error: mean={np.mean(errors):.1f} px, "
            f"P95={np.percentile(errors, 95):.1f} px"
        )

    ticks = [t["duration_ms"] for t in data.get("tick_times", [])]
    if ticks:
        print(f"  Tick Time:  mean={np.mean(ticks):.3f} ms, max={np.max(ticks):.3f} ms")
